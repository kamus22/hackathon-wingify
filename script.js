...
window.consolidatePDFs = async function() {
    ...
    consolidateBtn.textContent = '🔄 Processing PDFs...';
    ...
    try {
        ...
        resultDiv.innerHTML = `
            <div class="current">
                ...
                <p>📄 Consolidated report has been downloaded.</p>
            </div>
        `;
        ...
        consolidateBtn.textContent = '🔗 Consolidate & Generate Report';
    }
};
...
async function generateConsolidatedReport(sortedArticles, totalPDFs) {
    ...
    sortedArticles.forEach((item, index) => {
        ...
        doc.setFontSize(12);
        doc.setFont(undefined, 'bold');
        const priorityText = item.count > 1 ? `🔥 PRIORITY ${item.count}x` : `${index + 1}.`;
...
async function checkVWOArticle() {
    ...
    // Validate URL
    if (!url) {
        resultDiv.innerHTML = '<p class="error">⚠️ Please enter a VWO article URL</p>';
        return;
    }
    
    if (!url.includes('help.vwo.com')) {
        resultDiv.innerHTML = '<p class="error">⚠️ Please enter a valid VWO help article URL (must contain "help.vwo.com")</p>';
        return;
    }
    ...
    resultDiv.innerHTML = '<p class="info">🔄 Fetching article data...</p>';
    ...
    try {
        ...
        if (!updateDate) {
            ...
            resultDiv.innerHTML = `
                <div class="error">
                    <h4>❌ Could not find the update date</h4>
        ...
        if (daysDifference > oneYearInDays) {
            ...
            if (existingDraft) {
                ...
                resultDiv.innerHTML = `
                    <div class="outdated">
                        ...
                        <p>⚠️ This article needs to be reviewed and updated!</p>
                        <hr>
                        <p class="info">✓ This article has already been submitted for review.</p>
                ...
                resultDiv.innerHTML = `
                    <div class="outdated">
                        ...
                        <button onclick="simulateVWODraft('${articleTitle}', '${formattedDate}')" id="vwo-ai-draft-btn" style="margin-bottom: 10px;">🤖 Simulate AI Draft</button>
                        ...
                        <button onclick="addToExportListFromChecker()" style="margin-left: 10px;">📋 Add to Export List</button>
                    </div>
                `;
            }
        } else {
            ...
            resultDiv.innerHTML = `
                <div class="current">
                    <h4>✅ ARTICLE UP-TO-DATE</h4>
                    <p><strong>Article:</strong> ${articleTitle}</p>
                    <p><strong>Last Updated:</strong> ${formattedDate}</p>
                    <p><strong>Age:</strong> ${daysAgo} days old (${monthsAgo} months)</p>
                    <p>✓ This article has been updated within the last year.</p>
                    <button onclick="addToExportListFromChecker('${url.replace(/'/g, "\\'")}', '${articleTitle.replace(/'/g, "\\'")}', '${formattedDate}', ${daysAgo})">📋 Add to Export List</button>
                </div>
            `;
        }
...
// Simulate AI Draft for VWO Article Updates
...
function updateReviewPanel() {
    ...
    pendingDrafts.forEach(draft => {
        ...
        if (draft.isVWOArticle) {
            item.innerHTML = `
                <span>
                    <strong>🌐 VWO Article:</strong> ${draft.title}<br>
...
function reviewAction(action) {
    ...
    if (action === 'approve') {
        if (draft.isVWOArticle) {
            ...
            messageElement.textContent = `✅ VWO Article Acknowledged! Article "${draftTitle}" has been flagged for the documentation team to update.`;
        ...
        messageElement.textContent = `❌ VWO Article flag dismissed. No action will be taken.`;
            ...
            html += `
                <div class="export-item">
                    <div class="export-item-header">
                        ...
                        <button class="remove-btn" onclick="removeFromExportList(${index})">✕</button>
                    </div>
...
This VWO help article needs to be reviewed and updated to reflect current product features and UI.`,
    ...
    alert(`✅ Article "${title}" has been:\n• Added to your export list\n• Flagged for admin review`);
            ...
            html += `
                <div class="pdf-item">
                    <div class="pdf-item-header">
                        ...
                        <button class="remove-btn" onclick="removeUploadedPDF(${index})">✕</button>
                    </div>
...
window.simulateVWODraft = simulateVWODraft;
